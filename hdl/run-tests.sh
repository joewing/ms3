        ...
        for ((a=1; a<=8; a=a*2)) ; do
            for p in 'lru' 'mru' 'fifo' 'plru' ; do
                for wb in 'true' 'false' ; do
                    ...
                    echo "(memory $MAIN_MEM))"          >> input.mem
                    run
                done
            done
        done
...
for ((size=8; size<=256; size=size*2)) ; do
    echo "(spm (size $size) (memory $MAIN_MEM))" > input.mem
    run
done
...
echo "Testing offset..."
ghdl -a offset.vhdl
echo "(offset (value 8)(bank (join))(memory $MAIN_MEM))" > input.mem
run
echo "(offset (value -16)(bank (join))(memory $MAIN_MEM))" > input.mem
run
echo "(offset (value 3)(bank (join))(memory $MAIN_MEM))" > input.mem
run
